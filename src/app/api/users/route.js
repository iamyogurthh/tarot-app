...
export async function POST(req,res){
    ...
    let { topic, user_name, full_name, dob, major } =
        getDataFromForm(formData, 'topic', 'user_name', 'full_name', 'dob', 'major');

    let zodiac = getZodiacSign(dob);
    let numerology = getNumerology(dob);

    const ok = await createUser(user_name,full_name,dob,major,zodiac,numerology);
    if(!ok){
